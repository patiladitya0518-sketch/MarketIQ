...
def calculate_support_resistance(
    ...
    # Vectorized equivalent of the original centered-window checks.
    # The original used equality with the surrounding max/min, so ties are
    # intentionally preserved here as well. NaN handling is also preserved:
    # a non-finite candidate is never accepted as a level.
    ...
    window = lookback * 2 + 1
    high_roll = high_values.rolling(
        window=window, center=True, min_periods=1
    ).max()
    low_roll = low_values.rolling(
        window=window, center=True, min_periods=1
    ).min()

    valid_high = high_values.notna()
    valid_low = low_values.notna()

    interior = pd.Series(False, index=df.index)
    interior.iloc[lookback:len(df) - lookback] = True

    highs = high_values[
        interior & valid_high & high_values.eq(high_roll)
    ].tolist()

    lows = low_values[
        interior & valid_low & low_values.eq(low_roll)
    ].tolist()
