...
def detect_swings(
    ...
    # The original implementation required every value in the left/right
    # comparison windows to be finite. Keep that behavior while replacing
    # repeated DataFrame slicing with vectorized rolling operations.
    window = lookback
    left_high_max = pd.Series(high_values).rolling(window).max().shift(1).to_numpy()
    right_high_max = pd.Series(high_values).rolling(window).max().shift(-window).to_numpy()
    left_low_min = pd.Series(low_values).rolling(window).min().shift(1).to_numpy()
    right_low_min = pd.Series(low_values).rolling(window).min().shift(-window).to_numpy()

    high_finite_count_left = pd.Series(np.isfinite(high_values).astype(np.int8)).rolling(window).sum().shift(1).to_numpy()
    high_finite_count_right = pd.Series(np.isfinite(high_values).astype(np.int8)).rolling(window).sum().shift(-window).to_numpy()
    low_finite_count_left = pd.Series(np.isfinite(low_values).astype(np.int8)).rolling(window).sum().shift(1).to_numpy()
    low_finite_count_right = pd.Series(np.isfinite(low_values).astype(np.int8)).rolling(window).sum().shift(-window).to_numpy()

    highs = []
    lows = []

    for i in range(lookback, len(df) - lookback):
        current_high = _safe_float(high_values[i])
        current_low = _safe_float(low_values[i])

        if (
            np.isfinite(current_high)
            and high_finite_count_left[i] == window
            and high_finite_count_right[i] == window
            and current_high > left_high_max[i]
            and current_high > right_high_max[i]
        ):
            highs.append({"index": i, "price": current_high})

        if (
            np.isfinite(current_low)
            and low_finite_count_left[i] == window
            and low_finite_count_right[i] == window
            and current_low < left_low_min[i]
            and current_low < right_low_min[i]
        ):
            lows.append({"index": i, "price": current_low})
...
def detect_order_blocks(
    ...
    for i in range(start, len(df)):
        previous_open = _safe_float(opens[i - 1])
        previous_close = _safe_float(closes[i - 1])
        previous_high = _safe_float(highs[i - 1])
        previous_low = _safe_float(lows[i - 1])
        current_close = _safe_float(closes[i])

        if previous_close < previous_open and current_close > previous_high:
            bullish.append({
                "index": i - 1,
                "high": round(previous_high, 2),
                "low": round(previous_low, 2),
                "price": round(previous_low, 2),
                "type": "BULLISH_ORDER_BLOCK",
            })

        if previous_close > previous_open and current_close < previous_low:
            bearish.append({
                "index": i - 1,
                "high": round(previous_high, 2),
                "low": round(previous_low, 2),
                "price": round(previous_high, 2),
...
def detect_fvg(
    ...
    for i in range(start, len(df)):
        first_high = _safe_float(highs[i - 2])
        first_low = _safe_float(lows[i - 2])
        third_high = _safe_float(highs[i])
        third_low = _safe_float(lows[i])

        if third_low > first_high:
            bullish.append({
                "index": i,
                "low": round(first_high, 2),
                "high": round(third_low, 2),
                "type": "BULLISH_FVG",
            })
        ...
        if third_high < first_low:
            bearish.append({
                "index": i,
                "low": round(third_high, 2),
                "high": round(first_low, 2),
                "type": "BEARISH_FVG",
            })
